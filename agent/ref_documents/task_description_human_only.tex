...
{\small\itshape\color{grey77}This document is intended to be read by the engineer and sets the task scope.}\par
...
Make use of the certified tool \texttt{ductile-loads}, available as a Python package on PyPI (use uv). Notice that the customer has delivered the files in a different format (YAML) than expected by the tool (JSON). Part of your task is to bypass this limitation.
...
\begin{itemize}
  \item \textbf{This document} --- a printed copy for you and also available in the working directory. 
  \item \textbf{Strength Analysis Design Practive 1000} --- Overall product document describing all the steps, your task is a subset of all. It is important because it points to all other subdocuments tools available to be used, beyond this document.
  ...
  \item \textbf{Previous analysis} --- scripts, inputs and outputs in the \texttt{previous\_run/} folder for reference.
\end{itemize}
